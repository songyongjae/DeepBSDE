...
class DeepBSDE(tf.keras.Model):
    def __init__(self, f, x, g, sigma, mu, t0=0.0, t1=1.0, dim=100, time_steps=20,
        ...
        _bn = lambda: tf.keras.layers.BatchNormalization(momentum=.99, epsilon=1e-6,
    ...
    def draw_X_and_dW(self, num_sample):
        ...
        for i in range(self.N):
            t = self.t_space[i]
            X[:, :, i+1] = X[:, :, i] + self.mu(t, X[:, :, i]) * self.dt + tf.reduce_sum(
                self.sigma(t, X[:, :, i]) * dW[:, :, i], axis=1, keepdims=True)
